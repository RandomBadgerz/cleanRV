    ...
    if st.session_state.append_files_pressed:
        ...
        for col in columns:
            ...
            if col not in st.session_state.checkbox_states:
                ...
                if col in column_name_mapping_dict.keys():
        ...
        if selected_columns:
            ...
            filtered_df = combined_df[selected_columns].rename(columns=rename_mapping)
...
if st.session_state.combined_df is not None and st.session_state.columns_to_include:
    # Filter and rename the combined DataFrame
    rename_mapping = {col: st.session_state.rename_columns[col] for col in st.session_state.columns_to_include}
    filtered_combined_df = st.session_state.combined_df[st.session_state.columns_to_include].rename(
        columns=rename_mapping)

    # Export filtered combined DataFrame to an Excel file
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        filtered_combined_df.to_excel(writer, index=False, sheet_name='cleaned')
    ...
    if st.session_state.clean_files_pressed:
        ...
        cleaned_df = filtered_combined_df

        # add title to message
        cleaned_df['message'] = combined_df['title'].fillna('') + ' ' + cleaned_df['message'].fillna('')
        # cut content length to 750
        cleaned_df['message'] = cleaned_df['message'].str[:750]
        ...
        for i in source_dict:
            cleaned_df['channel'] = cleaned_df['channel'].replace(i, source_dict[i])

    # Add download button for Excel file
    st.download_button(
        label="Download Filtered Combined Excel File",
        data=processed_file,
        file_name="filtered_combined_files.xlsx",
